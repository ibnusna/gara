...
function startTimer(durationInMinutes, displayElement, onTimerEndCallback) {
  ...
  let serverOffset = 0;
  if (typeof window.__SERVER_TIMESTAMP__ !== 'undefined') {
    if (typeof window.__TIME_OFFSET__ === 'undefined') {
      window.__TIME_OFFSET__ = window.__SERVER_TIMESTAMP__ - Date.now();
    }
    serverOffset = window.__TIME_OFFSET__;
  }
