...
android {
    ...
    signingConfigs {
        create("release") {
            ...
            // Konfigurasi keystore untuk distribusi
        }
    }
    buildTypes {
        release {
            isMinifyEnabled = true      // ProGuard aktif
            isShrinkResources = true    // Resource shrinking aktif
        }
    }
    compileOptions {
...
dependencies {
    ...
    // Custom Tabs (Chrome / Browser in-app)
...
private fun setupWebView() {
    ...
    // Identifikasi: tambahkan User-Agent khusus agar backend bisa
    // mendeteksi request yang berasal dari aplikasi native ini
    settings.userAgentString += " GARA_OFFICIAL_APP"

    // Aktifkan fitur web modern
    settings.javaScriptEnabled = true        // Wajib untuk SPA/interaksi web
    settings.domStorageEnabled = true        // Aktifkan localStorage / sessionStorage
    settings.allowFileAccess = true          // Izinkan akses file lokal (asset)
    settings.allowContentAccess = true       // Izinkan akses content provider
    settings.databaseEnabled = true          // IndexedDB support

    // Nonaktifkan zoom (app didesain untuk portrait fixed)
    settings.setSupportZoom(false)
    settings.builtInZoomControls = false
    settings.displayZoomControls = false

    // Caching: gunakan mode default browser (cache jika valid)
    settings.cacheMode = WebSettings.LOAD_DEFAULT

    // Media: izinkan autoplay audio/video tanpa gesture user
    settings.mediaPlaybackRequiresUserGesture = false

    // Hardware acceleration untuk rendering yang mulus
...
cookieManager.setAcceptCookie(true)                         // Izinkan cookie first-party
cookieManager.setAcceptThirdPartyCookies(webView, true)     // Izinkan cookie third-party
```
...
```kotlin
// 1. Cegah long-press context menu pada elemen non-teks
webView.setOnLongClickListener {
    val hr = webView.hitTestResult
    hr.type != WebView.HitTestResult.EDIT_TEXT_TYPE
}

// 2. Inject CSS untuk disable user-select, KECUALI pada input/textarea
...
override fun onPageFinished(view: WebView?, url: String?) {
    ...
    // Simpan URL terakhir (bukan asset lokal)
...
// Saat activity masuk background
...
private fun activateExamMode(showFAB: Boolean = false) {
    if (!isExamMode) {
        ...
        // 1. FLAG_SECURE: Blokir screenshot & screen recording
        window.addFlags(WindowManager.LayoutParams.FLAG_SECURE)

        // 2. Kunci Volume pada level MAKSIMUM
        val audioManager = getSystemService(AUDIO_SERVICE) as AudioManager
        audioManager.setStreamVolume(
            AudioManager.STREAM_MUSIC,
            audioManager.getStreamMaxVolume(AudioManager.STREAM_MUSIC), 0
        )
        // Daftarkan observer untuk mencegah pengguna menurunkan volume
    ...
    // 4. Tampilkan/sembunyikan FAB berdasarkan tipe halaman ujian
    fabContainer.visibility = if (showFAB) View.VISIBLE else View.GONE

    // 5. Lock Task Mode: cegah pengguna keluar dari aplikasi
    try { startLockTask() } catch (_: Exception) {}

    // 6. Sembunyikan system bar (fullscreen immersive)
...
private fun deactivateExamMode() {
    if (isExamMode) {
        ...
        // Batalkan registrasi volume observer
        volumeObserver?.let {
...
override fun onWindowFocusChanged(hasFocus: Boolean) {
    ...
    if (isExamMode) {
        if (hasFocus) {
            ...
            // Jika window kehilangan fokus (notification panel terbuka,
            // overlay app muncul) → tampilkan layar hitam
...
inner class VolumeObserver(handler: Handler) : ContentObserver(handler) {
    override fun onChange(selfChange: Boolean) {
        ...
        val audioManager = getSystemService(AUDIO_SERVICE) as AudioManager
        // Setiap kali setting volume sistem berubah, langsung kembalikan ke maksimum
        audioManager.setStreamVolume(
            AudioManager.STREAM_MUSIC,
            audioManager.getStreamMaxVolume(AudioManager.STREAM_MUSIC), 0
        )
...
try {
    ...
    e.printStackTrace()    // Fallback: tampilkan ikon default tanpa crash
}
```
...
val clickThreshold = 10          // Pixel; jika bergerak < 10px, dianggap tap
...
fabMain.setOnTouchListener { _, event ->
    when (event.action) {
        ...
        MotionEvent.ACTION_MOVE -> {
            // Animasikan posisi container mengikuti jari
        ...
        MotionEvent.ACTION_UP -> {
            ...
            if (abs(endX - startX) < clickThreshold && abs(endY - startY) < clickThreshold) {
                toggleFabMenu()    // Jika minimal movement → toggle menu
            }
            true
        }
    }
...
private fun toggleFabMenu() {
    ...
    if (isFabMenuOpen) {
        ...
        fabMain.alpha = 1.0f                // Opacity penuh saat dibuka
    } else {
        fabSubExit.visibility = View.GONE
        fabSubRefresh.visibility = View.GONE
        fabMain.alpha = 0.5f               // Semi-transparan saat tertutup (glass effect)
    }
}
```
...
private fun exitExamAndGoToDashboard() {
    ...
    webView.clearHistory()          // Hapus history agar tombol back tidak kembali ke ujian
    webView.clearCache(true)
    deactivateExamMode()
    webView.loadUrl(mainHomeUrl)    // Kembali ke dashboard utama
}
```
...
override fun shouldOverrideUrlLoading(
    ...
    // 1. Blokir YouTube (kecuali halaman akun Google)
    if ((url.contains("youtube.com") || url.contains("youtu.be"))
        && !url.contains("accounts.")) {
        Toast.makeText(context, "Akses ke YouTube diblokir.", Toast.LENGTH_SHORT).show()
        return true    // true = intercept, jangan muat
    }

    // 2. Cek apakah URL diizinkan dimuat di WebView internal
    ...
    if (isAllowedInternal) return false    // false = biarkan WebView yang muat

    // 3. URL eksternal lainnya → buka di Chrome Custom Tab
    try {
...
| `file:///android_asset/` | WebView internal | Asset lokal (eror.html) |
...
onBackPressedDispatcher.addCallback(this, object : OnBackPressedCallback(true) {
    override fun handleOnBackPressed() {
        // Prioritas 1: Keluar dari mode fullscreen video
        if (customView != null) {
            mWebChromeClient.onHideCustomView()
            return
        }
        // Prioritas 2: Jika blackout aktif, abaikan back press
        ...
        // Prioritas 3: Kunci back saat ujian / di halaman third-party
        ...
        // Prioritas 4: Navigasi web normal
        if (webView.canGoBack()) {
            webView.goBack()
        } else {
            isEnabled = false
            onBackPressedDispatcher.onBackPressed()    // Keluar dari app
        }
    }
})
```
...
private fun setupScrollSync() {
    webView.viewTreeObserver.addOnScrollChangedListener {
        swipeRefreshLayout.isEnabled =
            if (isExamMode) false       // Nonaktif total saat ujian
            else webView.scrollY == 0   // Aktif hanya di posisi paling atas
    }
}
```
        ...
        xhr.onload = function(e) {
            if (this.status == 200) {
                ...
                reader.onloadend = function() {
                    // Panggil JavascriptInterface untuk menyimpan file
...
private fun saveBase64ToFile(base64String: String, fileName: String) {
    val base64Data = if (base64String.contains(","))
        base64String.split(",")[1] else base64String   // Hapus prefix "data:application/pdf;base64,"
    ...
    // Pemberitahuan ke Media Scanner agar file muncul di file manager
...
override fun onReceivedError(
    ...
    if (request?.isForMainFrame == true
        ...
        view?.loadUrl("file:///android_asset/helpers/eror.html")
    }
}
```
...
function handleRetry() {
    ...
    if (window.AndroidInterface) {
        // Panggil Kotlin untuk memuat ulang URL terakhir yang valid
        window.AndroidInterface.retryLastUrl();
    } else {
        // Fallback jika dibuka di browser biasa (development testing)
...
function submitLocalUrl() {
    ...
    if (window.AndroidInterface) {
        // Simpan URL ke SharedPreferences & navigasi langsung
